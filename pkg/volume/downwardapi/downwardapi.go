...
func (d *downwardAPIVolume) collectData() (map[string][]byte, error) {
	...
	for _, fileInfo := range d.items {
		if fileInfo.FieldRef != nil && fileInfo.ContainerSpecFieldRef != nil {
			return data, fmt.Errorf("both fieldRef and containerFieldRef can not be specified simultaneously")
		}
		...
		} else if fileInfo.ContainerSpecFieldRef != nil {
			if values, err := fieldpath.ExtractJSONFieldSelectorValueForContainer(fileInfo.ContainerSpecFieldRef, d.pod, fileInfo.ContainerSpecFieldRef.Name); err != nil {
				glog.Errorf("Unable to extract field %s: %s", fileInfo.ContainerSpecFieldRef.FieldPath, err.Error())
				errlist = append(errlist, err)
			} else {
				data[path.Clean(fileInfo.Path)] = []byte(sortLines(values))
			}
